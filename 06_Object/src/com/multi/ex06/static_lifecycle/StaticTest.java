...
//
        int count = 0;
        while(true) {
            new User("홍길동" + count++); // 메모리에 쌓여있는 객체
        }
    }
}
...
//		name = "홍길동"; // Cannot make a static reference to the non-static field name
    ...
    { // 객체 생성시 최우선 호출되는 영역
        ...
        if (currentUserNumber > MAX_SIZE) {
            ...
            System.gc(); // Runs the garbage collector in the Java Virtual Machine. -> gc가 있을 때 없을때를 비교해볼것
            // 메모리 정리를 요청하는 메소드

            try { // 이게 없으면 소멸하지 않는다.
                Thread.sleep(1000); // 1초간 기다리는 코드 -> 통신할때 sleep 필요함
